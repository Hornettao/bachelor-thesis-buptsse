...
var qb = module.exports = function (gconn) {
  ...
  return function (sql, params, gconv) {
    ...
    /**
     * @desc Query Executor
     * @param conn {Connection} (optiinal) query with given connection
     * @param args {Object} arguments passed to generate sql command
     * @param conv {Object} (optional) query specific value converter chain,
     *                      will be concat to main chain specified from factory
     * @param cb {Function} (optional) callback function (stream mode if not specified)
     */
    return function (conn, arg, conv, cb) {

      if (!~['PoolConnection', 'Connection'].indexOf(conn.constructor.name)) {
        cb = conv; conv = arg; arg = conn; conn = null;
      }
      ...
      var perform = function (conn, release) {
      ...
      if (conn) {
        // Run at given Connection
        perform(conn, function () { });
      } else {
        // Obtain a Connection
        gconn(function (err, conn) {
          if (err) return cb(err);
          perform(conn, function () { conn.release(); });
        });
      }
    ...
    return function (values) {
      ...
      return template.replace(/\:\:(\w+)/g, function (txt, key) {
        ...
        if (values.hasOwnProperty(key)) {
          ...
          if (typeid(values[key]) == 'array') {
            return '(' + _.map(params[key](values[key]), mysql.escape).join(', ') + ')';
          } else {
